...
export class Migration20260325092009 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`alter table if exists "site_config" drop constraint if exists "site_config_key_unique";`);
    this.addSql(`create table if not exists "site_config" ("id" text not null, "key" text not null, "value" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "site_config_pkey" primary key ("id"));`);
