...
#
# Install on the server (every 6 hours):
#   0 */6 * * * /opt/repositories/garden-for-you-monorepo/scripts/monitor/check-backups.sh
...
if [ -z "$LATEST" ]; then
    notify "⚠️ В бакете revus-garden-backups вообще нет бэкапов!"
    exit 1
fi
...
if [ -z "$FILE_TIMESTAMP" ]; then
    notify "⚠️ Не удалось определить дату последнего бэкапа"
    exit 1
fi
...
if [ "$AGE_HOURS" -gt "$MAX_AGE_HOURS" ]; then
    notify "🚨 Последний бэкап был $AGE_HOURS часов назад!
